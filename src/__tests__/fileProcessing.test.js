import { describe, it, expect, beforeAll } from 'vitest';
// Use legacy build for Node.js environment (no DOMMatrix required)
import * as pdfjsLib from 'pdfjs-dist/legacy/build/pdf.mjs';
import mammoth from 'mammoth';
import { readFileSync } from 'fs';
import { join } from 'path';

// Helper to create a mock File object from buffer
function createMockFile(buffer, name, type) {
    const blob = new Blob([buffer], { type });
    return new File([blob], name, { type });
}

// Minimal 1x1 transparent PNG (base64)
const MINIMAL_PNG_BASE64 = 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg==';

// Create a minimal PNG buffer
function createPngBuffer() {
    return Buffer.from(MINIMAL_PNG_BASE64, 'base64');
}

// Create a minimal DOCX buffer (this is a valid minimal docx structure)
async function createDocxBuffer() {
    // Read actual test DOCX file if exists, otherwise create minimal structure
    const JSZip = (await import('jszip')).default;
    const zip = new JSZip();

    // Minimal DOCX structure
    const contentTypesXml = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types">
  <Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/>
  <Default Extension="xml" ContentType="application/xml"/>
  <Override PartName="/word/document.xml" ContentType="application/vnd.openxmlformats-officedocument.wordprocessingml.document.main+xml"/>
</Types>`;

    const relsXml = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="word/document.xml"/>
</Relationships>`;

    const documentXml = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
  <w:body>
    <w:p>
      <w:r>
        <w:t>Contenu de test pour le document Word.</w:t>
      </w:r>
    </w:p>
  </w:body>
</w:document>`;

    zip.file('[Content_Types].xml', contentTypesXml);
    zip.file('_rels/.rels', relsXml);
    zip.file('word/document.xml', documentXml);

    return await zip.generateAsync({ type: 'arraybuffer' });
}

describe('File Processing', () => {
    describe('Image Processing', () => {
        it('should read PNG image as base64', async () => {
            const pngBuffer = createPngBuffer();
            const file = createMockFile(pngBuffer, 'test.png', 'image/png');

            // Simulate readFileAsBase64
            const reader = new FileReader();
            const base64Promise = new Promise((resolve, reject) => {
                reader.onload = () => resolve(reader.result);
                reader.onerror = reject;
                reader.readAsDataURL(file);
            });

            const result = await base64Promise;

            expect(result).toBeDefined();
            expect(result).toMatch(/^data:image\/png;base64,/);
            expect(result.length).toBeGreaterThan(0);
        });

        it('should read JPEG image as base64', async () => {
            // Minimal JPEG buffer (1x1 white pixel)
            const jpegBuffer = Buffer.from([
                
            ]);

            const file = createMockFile(jpegBuffer, 'test.jpg', 'image/jpeg');

            const reader = new FileReader();
            const base64Promise = new Promise((resolve, reject) => {
                reader.onload = () => resolve(reader.result);
                reader.onerror = reject;
                reader.readAsDataURL(file);
            });

            const result = await base64Promise;

            expect(result).toBeDefined();
            expect(result).toMatch(/^data:image\/jpeg;base64,/);
        });
    });

    describe('PDF Processing', () => {
        let pdfArrayBuffer;

        beforeAll(async () => {
            // Read the test PDF file
            const pdfPath = join(__dirname, 'fixtures', 'test.pdf');
            try {
                const fileBuffer = readFileSync(pdfPath);
                // Convert Node.js Buffer to Uint8Array for pdfjs
                pdfArrayBuffer = new Uint8Array(fileBuffer);
            } catch {
                // If file doesn't exist, skip PDF tests
                pdfArrayBuffer = null;
            }
        });

        it('should extract text from PDF using pdfjs-dist', async () => {
            if (!pdfArrayBuffer) {
                console.log('Skipping PDF test - no test PDF file found');
                return;
            }

            // Disable worker for Node.js environment
            const loadingTask = pdfjsLib.getDocument({
                data: pdfArrayBuffer,
                useWorkerFetch: false,
                isEvalSupported: false,
                useSystemFonts: true,
            });
            const pdf = await loadingTask.promise;

            expect(pdf.numPages).toBeGreaterThan(0);

            const page = await pdf.getPage(1);
            const textContent = await page.getTextContent();

            expect(textContent).toBeDefined();
            expect(textContent.items).toBeDefined();

            // Verify we can extract actual text
            const textItems = textContent.items.map(item => item.str).join(' ');
            expect(textItems.length).toBeGreaterThan(0);
            expect(textItems).toContain('PDF');
        });
    });

    describe('DOCX Processing', () => {
        it('should extract text from DOCX using mammoth', async () => {
            const docxArrayBuffer = await createDocxBuffer();
            // Convert ArrayBuffer to Buffer for Node.js mammoth
            const docxBuffer = Buffer.from(docxArrayBuffer);

            const result = await mammoth.extractRawText({ buffer: docxBuffer });

            expect(result).toBeDefined();
            expect(result.value).toBeDefined();
            expect(result.value).toContain('Contenu de test pour le document Word');
        });

        it('should handle DOCX with special characters', async () => {
            const JSZip = (await import('jszip')).default;
            const zip = new JSZip();

            const contentTypesXml = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types">
  <Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/>
  <Default Extension="xml" ContentType="application/xml"/>
  <Override PartName="/word/document.xml" ContentType="application/vnd.openxmlformats-officedocument.wordprocessingml.document.main+xml"/>
</Types>`;

            const relsXml = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="word/document.xml"/>
</Relationships>`;

            // Document with French special characters
            const documentXml = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
  <w:body>
    <w:p>
      <w:r>
        <w:t>Caractères spéciaux: é è à ù ç ê â î ô û ë ï ü</w:t>
      </w:r>
    </w:p>
  </w:body>
</w:document>`;

            zip.file('[Content_Types].xml', contentTypesXml);
            zip.file('_rels/.rels', relsXml);
            zip.file('word/document.xml', documentXml);

            const docxArrayBuffer = await zip.generateAsync({ type: 'arraybuffer' });
            // Convert ArrayBuffer to Buffer for Node.js mammoth
            const docxBuffer = Buffer.from(docxArrayBuffer);
            const result = await mammoth.extractRawText({ buffer: docxBuffer });

            expect(result.value).toContain('é');
            expect(result.value).toContain('ç');
        });
    });

    describe('Text File Processing', () => {
        it('should read plain text file', async () => {
            const textContent = 'Ceci est un fichier texte de test.';
            const file = createMockFile(
                Buffer.from(textContent, 'utf-8'),
                'test.txt',
                'text/plain'
            );

            const reader = new FileReader();
            const textPromise = new Promise((resolve, reject) => {
                reader.onload = () => resolve(reader.result);
                reader.onerror = reject;
                reader.readAsText(file);
            });

            const result = await textPromise;

            expect(result).toBe(textContent);
        });

        it('should read markdown file', async () => {
            const mdContent = '# Titre\n\nCeci est un paragraphe avec du **gras**.';
            const file = createMockFile(
                Buffer.from(mdContent, 'utf-8'),
                'test.md',
                'text/markdown'
            );

            const reader = new FileReader();
            const textPromise = new Promise((resolve, reject) => {
                reader.onload = () => resolve(reader.result);
                reader.onerror = reject;
                reader.readAsText(file);
            });

            const result = await textPromise;

            expect(result).toBe(mdContent);
            expect(result).toContain('# Titre');
        });
    });

    describe('File Type Detection', () => {
        it('should identify image types correctly', () => {
            const imageTypes = [
                'image/png',
                'image/jpeg',
                'image/gif',
                'image/webp',
                'image/svg+xml'
            ];

            imageTypes.forEach(type => {
                expect(type.startsWith('image/')).toBe(true);
            });
        });

        it('should identify PDF type correctly', () => {
            const pdfType = 'application/pdf';
            expect(pdfType).toBe('application/pdf');
        });

        it('should identify DOCX type correctly', () => {
            const docxType = 'application/vnd.openxmlformats-officedocument.wordprocessingml.document';
            expect(docxType).toBe('application/vnd.openxmlformats-officedocument.wordprocessingml.document');
        });
    });
});
